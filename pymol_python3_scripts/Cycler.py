...
class PDBDirCyclerLite(LoadDeleteCycler):
    def __init__(self,target_dir='.'):
        ...
        self.pdbs = [ f for f in os.listdir(target_dir) if re.search('.pdb$|.pdb.gz$',f) ]
        pdb = self.pdbs[0]
        cmd.load(pdb)
    def choices(self):
        return self.pdbs
...
def keep_pdb(): # Swipe Right!
    create_dir("keep")
    print('What a beautiful protein!')
    current_pdb=cmd.get_object_list()[0]

    if os.path.exists(f"{current_pdb}.pdb"):
        subprocess.call(["mv", f"{current_pdb}.pdb", "keep/"])

    elif os.path.exists(f"{current_pdb}.pdb.gz"):
        subprocess.call(["mv", f"{current_pdb}.pdb.gz", "keep/"])
    else:
        return None

    viewing.cycler.iter(1)

def discard_pdb(): # Swipe Left!
    create_dir("discard")
    print('Maybe another time...')
    current_pdb=cmd.get_object_list()[0]

    if os.path.exists(f"{current_pdb}.pdb"):
        subprocess.call(["mv", f"{current_pdb}.pdb", "discard/"])

    elif os.path.exists(f"{current_pdb}.pdb.gz"):
        subprocess.call(["mv", f"{current_pdb}.pdb.gz", "discard/"])
    else:
        return None
...
def spawnPDBDirCycler(target_dir='.'):
    ...
    cmd.set_key('pgup',keep_pdb)
    cmd.set_key('pgdn',discard_pdb)
...
cmd.extend( 'pdbcycler', lambda dir='.': spawnPDBDirCycler('.') )
